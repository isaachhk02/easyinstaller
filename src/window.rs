...
use dialog::{Choice, DialogBox, Message, Question};
pub fn installation_type(language: &str, package_manager: &str, package_path: &str, its_arch: i32) {
    let uninstall;
    let install_or_remove_package_msgs = match language {
        "en_US" => {
            "Do you want to install or remove a package ? (press yes to install or press no to uninstall)"
        }
        "fr_FR" => "Voulez-vous installer ou supprimer un paquet?",
        "es_ES" => {
            "¿Quieres instalar o eliminar un paquete? (Pulsa Si para instalar el paquete de lo contrario pulsa No para eliminarlo"
        }
        _ => {
            "Do you want to install or remove a package ? (press yes to install or press no to uninstall)"
        }
    ...
    let install_or_remove_package = Question::new(install_or_remove_package_msgs)
        .title("Do you want?")
        .show();
    if install_or_remove_package.is_ok() {
        match install_or_remove_package {
            Ok(Choice::Yes) => {
                uninstall = 1;
                welcome_screen(language, package_manager, package_path, uninstall, its_arch);
                ()
            }
...
pub fn welcome_screen(
    ...
    let package_path = match package_manager {
        ...
        "pacman" => format!("file://{}", package_path),
    ...
    if welcomemsg.is_ok() {
        if uninstall == 1 {
            ...
            match uninstallmsg {
                Ok(Choice::Yes) => {
                    uninstall_package(language, package_manager, &package_path, its_arch);
                }
                Ok(Choice::No) | Ok(Choice::Cancel) => {
            ...
            match installmsg {
                Ok(Choice::Yes) => {
                    install_package(language, package_manager, &package_path, its_arch);
                }
                Ok(Choice::No) | Ok(Choice::Cancel) => {
    ...
    pub fn uninstall_package(
        ...
        let mut _command = format!("");
        if its_arch == 1 {
            let mut _command = format!("{} -Rc {} --noconfirm", package_manager, package_path);
        } else {
            let mut _command = format!("{} remove {} -y", package_manager, package_path);
